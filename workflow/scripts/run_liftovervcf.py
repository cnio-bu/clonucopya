...
if __name__ == '__main__':
    ...
    for idx_cluster_tsv, row_cluster_tsv in np.ndenumerate(arr):
        ...
        for idx, x in np.ndenumerate(arr):
            ...
            for index, row in df_cluster.iterrows():
                ...
                vep_file = 'ensembl_{}.{}.vcf'.format(sample_id, cluster_id)
                # print('>>>>>>>>>>>>>>{}'.format(vep_file))

                # run liftOverVcf
                # java -jar picard.jar LiftoverVcf I=input.vcf O=lifted_over.vcf CHAIN=b37tohg38.chain \
                #             REJECT=rejected_variants.vcf R=reference_sequence.fasta

                command = ["picard", "LiftoverVcf", "I=input.vcf", "CHAIN=b37tohg38.chain",
                           "REJECT=rejected_variants.vcf", "R=reference_sequence.fasta"]
    ...
    for index_pc, row_pc in df_patient_cluster_tsv.iterrows():
        ...
        #                       "./vep --cache --cache_version 104 --offline --format ensembl --vcf --force_overwrite \
        #                       -dir_cache /opt/vep/.vep --input_file {} --output_file {} --assembly GRCh38".format(
