...
if __name__ == '__main__':
    ...
    for idx_h5, row_h5 in np.ndenumerate(arr_h5):
        ...
        variant_assignment.to_csv(file_name.replace(".h5", "_variant_assignment.csv"), index=False, header=True, sep=",")
        ...
        arr_loci = np.array(loci_tsv)
        # iterate all input loci.tsv filenames
        for idx_loci_tsv, row_loci_tsv in np.ndenumerate(arr_loci):

            mutations = pd.read_csv(row_loci_tsv, sep="\t", header=0, low_memory=False)
            ...
            df_frames.append(pd.DataFrame(
                ...
                'VAF': mutations["variant_allele_frequency"]},
