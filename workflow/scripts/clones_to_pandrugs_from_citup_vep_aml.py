            ...
            if response.status_code == 201:
                ...
                while not exit_loop:
                    try:
                        ...
                        print('      Status generation of the VScore file --> status_code = {}'.format(
                            response.status_code))
                        r_json = response.json()
                        failed = r_json["failed"]
                        finished = r_json["finished"]
                        print('      Status generation of the VScore file --> failed={} and finished={}'.format(failed,
                                                                                                                finished))
                ...
                if sta_code == 200 and finished and not failed:
                    ...
                    filename = str(output_dir) + '/{}/{}/{}'.format(sample_id, cluster_id, str(get_filename_from_cd(
                        r.headers.get('Content-Disposition'))).replace('"', ''))
