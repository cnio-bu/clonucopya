...
if __name__ == '__main__':
    ...
    for idx_sample, row_sample in np.ndenumerate(arr_samples):
        ...
        for idx_smp_clone, row_smp_clone in np.ndenumerate(arr_sample_clones):
            ...
            if Path(vep_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + vep_file).exists():
                ...
                Path(vep_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + vep_file.replace('.tsv',
                                                                                                     '.vcf')).write_text(
                    "#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO")
                os.chmod(vep_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + vep_file.replace('.tsv', '.vcf'),
                         0o777)
                output_ensembl = "/opt/vep/.vep/" + str(patient_id) + '/' + sample_id + '/' + vep_file.replace('.tsv',
                                                                                                               '.vcf')

                # run docker
                client.containers.run("ensemblorg/ensembl-vep:release_104.3",
                                      "./vep --cache --cache_version 104 --offline --format ensembl --vcf --force_overwrite -dir_cache /opt/vep/.vep/{} --input_file {} --output_file {} --assembly GRCh37".format(
