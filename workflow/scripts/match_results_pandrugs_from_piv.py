...
if __name__ == '__main__':
    ...
    for idx_sample, row_sample in np.ndenumerate(arr_samples):
        ...
        for idx_smp_clone, row_smp_clone in np.ndenumerate(arr_sample_clones):
            ...
            path_computation_tsv = pandrugs_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + str(cluster_id) \
                                   + '/computation.tsv'
            if os.path.exists(path_computation_tsv):
                ...
                path_vscore_tsv = pandrugs_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + str(cluster_id) + '/'\
                ...
                for idx_vscore, row_vscore in vscore_tsv.iterrows():
                    ...
                    # get VAF value from mutations.tsv
                    # print('loc={} chr={} case={}'.format(vscore_loc, vscore_chr, sample_id))
                    # print(mutations_tsv.loc[(mutations_tsv['loc'] == vscore_loc) & (mutations_tsv['case'] == sample_id) & (mutations_tsv['chr'] == str(vscore_chr))])
                    mutations_tsv_pos = pd.DataFrame(mutations_tsv.loc[((mutations_tsv['chr'] == str(vscore_chr)) & (mutations_tsv['loc'] == vscore_loc) & (mutations_tsv['case'] == sample_id))])
                    clone_column.append(cluster_id)
                    # print(mutations_tsv_pos)
                    VAF_column.append(mutations_tsv_pos['VAF'].values[0])

                vscore_tsv['clone'] = clone_column
                vscore_tsv['VAF'] = VAF_column

                # i.e: /home/cleon/PycharmProjects/MSc-Project/out/heatmap/UPN933124
                if not os.path.exists(output_dir + '/' + str(patient_id)):
                    os.makedirs(output_dir + '/' + str(patient_id))

                # i.e: /home/cleon/PycharmProjects/MSc-Project/out/heatmap/UPN933124/tumor
                if not os.path.exists(output_dir + '/' + str(patient_id) + '/' + str(sample_id)):
                    os.makedirs(output_dir + '/' + str(patient_id) + '/' + str(sample_id))

                # i.e: /home/cleon/PycharmProjects/MSc-Project/out/heatmap/UPN933124/tumor/0
                ...
                vscore_tsv.fillna(0)
                vscore_tsv.to_csv(str(output_dir + '/{}/{}/{}/{}-vscore.ann.tsv').format(patient_id, sample_id,
                                                                                         cluster_id, computation_id),
                                  sep='\t',
                                  index=False)
            else:
                # delete from array
                np.delete(arr_sample_clones, cluster_id)
        ...
        for idx_smp_clone, row_smp_clone in np.ndenumerate(arr_sample_clones):
            ...
            if os.path.exists(path_computation_tsv):
                ...
                path_gen_drug_csv = pandrugs_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + str(cluster_id) + '/' + 'gene-drug.csv'
                if os.path.exists(path_gen_drug_csv) and os.path.getsize(path_gen_drug_csv) > 0:
                    print(path_gen_drug_csv)
                    gen_drug_csv = pd.read_csv(path_gen_drug_csv, sep=',')

                    gd_clone_column = []
                    gd_VAF_column = []

                    path_vscore_ann_tsv = output_dir + '/' + str(patient_id) + '/' + str(sample_id) + '/' + str(cluster_id) + '/' \
                    ...
                    gen_drug_csv.to_csv(str(output_dir + '/{}/{}/{}/gene-drug.ann.tsv').format(patient_id, sample_id, cluster_id),
                                        sep='\t',
                                        index=False)
