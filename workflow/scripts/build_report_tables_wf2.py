...
def build_gene_alterations(tree_df, pandrugs_dir, out_dir):
    ...
    study_df.columns = ['Mutation ID', 'Gene Symbol', 'Ensembl ID', 'Consequence', 'Impact', 'Clone']
    ...
    study_sorted = study_sorted[['Clone', 'Mutation ID', 'Gene Symbol', 'Ensembl ID', 'Consequence', 'Impact']]

    # Save to TSV table
    study_sorted.to_csv(f"{out_dir}/gene_alterations_wf2.tsv", sep='\t', index=False)
...
def build_drug_prioritization(gene_alterations, pandrugs_dir, out_dir):
    ...
    if concatenated_df.empty:
        ...
        study_subset = gene_alterations[['Clone', 'Mutation ID', 'Gene Symbol']]
        ...
        drug_prioritization = drug_prioritization.drop_duplicates(subset=['Clone', 'Mutation ID', 'Gene Symbol', 'Drug'])        
        
        drug_prioritization.to_csv(f"{out_dir}/drug_prioritization_wf2.tsv", sep='\t', index=False, na_rep='-')
...
if __name__ == '__main__':
    ...
    gene_alterations = build_gene_alterations(args.tree_df,args.pandrugs_dir, args.out_dir)
